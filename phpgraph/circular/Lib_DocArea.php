...
class Graph {
	...
	{
		if(is_array($this->values1[$row1]))
			return $this->values1[$row1];
	...
	{
		$vals1 = (is_array($this->values1[$row1]) ? $this->values1[$row1] : $this->values1);
		$k = array_keys($vals1);
	...
	{
		if(is_array($this->values1[0]))
			return max($this->values1[0]);
		return max($this->values1);
	}
